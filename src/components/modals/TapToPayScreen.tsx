  ...
  const stateCopy = useMemo(() => {
    switch (paymentState) {
      case "waiting_for_card":
        return { instruction: "Tap card behind device", showAmount: true, showBrands: true };
      case "processing":
        return { instruction: "Processing payment...", showAmount: true, showBrands: false };
      case "success":
        return { title: "Payment Successful", subtitle: `$${amount.toFixed(2)} has been charged`, showAmount: false, showBrands: false };
      case "failed":
        return {
      ...
      default:
        return { instruction: "", showAmount: false, showBrands: false };
    }
    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-white safe-area">
      ...
      <div className="relative flex min-h-screen flex-col items-center bg-white px-6 pt-16 pb-8">
        {paymentState === "authorizing" ? (
          ...
          <div className="flex flex-col items-center gap-3 text-center">
            <div className={cn("flex h-14 w-14 items-center justify-center rounded-full", paymentState === "success" ? "bg-green-100" : "bg-red-100")}> 
      ...
      {(paymentState === "success" || paymentState === "failed") && (
        <div className="shrink-0 px-6 pb-6 space-y-2">
          {paymentState === "success" && (
            <button onClick={handleDone} className="w-full rounded-lg bg-green-600 py-2 text-sm font-medium text-white transition-colors hover:bg-green-700 active:bg-green-800">
              Done
            </button>
          )}

          {paymentState === "failed" && cancelledPayment && (
            <button onClick={handleAcknowledgeCancelledPayment} className="w-full rounded-lg bg-orange-500 py-2 text-sm font-medium text-white transition-colors hover:bg-orange-600 active:bg-orange-700">
              OK
            </button>
          )}
          ...
          {paymentState === "failed" && !cancelledPayment && (
            <>
              <button onClick={handleRetry} className="w-full rounded-lg bg-orange-500 py-2 text-sm font-medium text-white transition-colors hover:bg-orange-600 active:bg-orange-700">
                Try Again
              </button>
              <button onClick={handleClose} className="w-full rounded-lg border border-gray-300 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 active:bg-gray-100">
                Cancel
              </button>
            </>
          )}
