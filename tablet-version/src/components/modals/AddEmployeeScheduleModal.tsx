...
interface AddEmployeeScheduleModalProps {
  open: boolean;
...
const AddEmployeeScheduleModal = ({
  ...
  currentEmployeeId,
  currentEmployee,
}: AddEmployeeScheduleModalProps) => {
  // EMPLOYEE LOGIN: Pre-fill with logged-in employee
  const [selectedEmployeeId, setSelectedEmployeeId] = useState(
    isEmployee && currentEmployeeId ? currentEmployeeId : ""
  );
  const [selectedEmployeeName, setSelectedEmployeeName] = useState(
    isEmployee && currentEmployee ? currentEmployee.name : ""
  );
  ...
  useEffect(() => {
    if (!open) {
      // EMPLOYEE LOGIN: Reset to logged-in employee, MERCHANT: Reset to empty
  ...
  }, [open, isEmployee, currentEmployeeId, currentEmployee]);

  // EMPLOYEE LOGIN: Pre-fill employee field when modal opens
  useEffect(() => {
    if (open && isEmployee && currentEmployeeId && currentEmployee) {
      setSelectedEmployeeId(currentEmployeeId);
      setSelectedEmployeeName(currentEmployee.name);
    }
  }, [open, isEmployee, currentEmployeeId, currentEmployee]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md w-[92%] max-h-[90vh] overflow-y-auto rounded-xl p-0 bg-white [&>button]:hidden shadow-xl">
        ...
        {/* Footer Actions - Orange Add Schedule Button */}
