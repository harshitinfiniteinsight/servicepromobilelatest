...
const ACHSetupSliderModal = ({ isOpen, onClose, onBack }: ACHSetupSliderModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-lg w-[calc(100%-2rem)] p-0 gap-0 rounded-2xl max-h-[85vh] overflow-hidden [&>div]:p-0 [&>button]:hidden flex flex-col">
        ...
        <div className="bg-orange-500 px-4 py-3 flex items-center justify-between safe-top shrink-0">
          <button
            onClick={onBack}
          ...
          <h2 className="text-lg font-semibold text-white px-2 text-center flex-1">ACH Setup Guide</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 bg-white flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-h-0 relative">
                ...
                {steps.map((step, index) => (
                  <CarouselItem key={index} className="h-full pl-0">
                    <div className="h-full flex flex-col px-6 pb-4">
                      {/* Screenshot Section */}
                      <div className={`w-full aspect-[4/3] rounded-xl ${step.placeholderColor} flex items-center justify-center mb-5 overflow-hidden shadow-sm`}>
                        {step.image ? (
                          <img
                            src={step.image}
                          ...
                          <div className="flex flex-col items-center justify-center text-gray-500">
                            <div className="w-20 h-20 rounded-full bg-white/80 flex items-center justify-center mb-2 shadow-sm">
            ...
            <button
              ...
              className={`absolute left-2 top-1/3 -translate-y-1/2 p-2 rounded-full bg-white/90 shadow-md transition-all ${
                currentStep === 0
            ...
            <button
              ...
              className={`absolute right-2 top-1/3 -translate-y-1/2 p-2 rounded-full bg-white/90 shadow-md transition-all ${
        ...
        {/* Sticky CTA Footer */}
        <div
          className="bg-white px-6 pb-7 pt-4 border-t border-gray-100 shrink-0 safe-bottom flex justify-center"
          style={{ paddingBottom: "calc(28px + env(safe-area-inset-bottom))" }}
        >
          <Button
            onClick={handleEmailInstructions}
            disabled={isSendingEmail}
            className="w-full max-w-[360px] h-12 text-base font-semibold rounded-xl bg-orange-500 hover:bg-orange-600 text-white disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSendingEmail ? (
