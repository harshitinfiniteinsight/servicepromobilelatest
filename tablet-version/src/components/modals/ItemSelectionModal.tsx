...
import type { InvoiceLineItem } from "@/utils/refundUtils";
...
function getMaxRefundQty(item: InvoiceLineItem): number {
  ...
  if (item.unitPrice > 0) {
    ...
    const maxByAmount = Math.floor(refundableAmt / item.unitPrice);
    return Math.max(1, Math.min(item.quantity, maxByAmount));
  }
...
const ItemSelectionModal = ({
  ...
  useEffect(() => {
    ...
    lineItems.forEach((item) => {
      if (item.total <= (item.refundedAmount ?? 0)) return;
      const maxQty = getMaxRefundQty(item);
      const restoredQty = initialItemQtys?.[item.itemId] ?? maxQty;
      initQtys[item.itemId] = Math.max(1, Math.min(restoredQty, maxQty));
    });
  ...
  const handleConfirm = () => {
    ...
    const itemsToConfirm: InvoiceLineItem[] = selectedItems.map((item) => {
      const maxQty = getMaxRefundQty(item);
      const rawQty = itemQtys[item.itemId] ?? maxQty;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[560px] w-[calc(100%-2rem)] mx-auto p-0 gap-0 rounded-2xl max-h-[85vh] overflow-hidden !flex !flex-col [&>button]:hidden">
        ...
        >
          <div className="px-6 py-5 pb-6 space-y-4 pointer-events-auto">
            {/* Invoice Reference */}
            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
              <p className="text-xs text-gray-500 font-semibold uppercase tracking-wide">Invoice</p>
              <p className="text-sm font-bold text-gray-900 mt-1">{invoiceId}</p>
              <p className="text-xs text-gray-500 mt-0.5 truncate">{invoiceName}</p>
            </div>
            ...
            {!isLoading && refundableItems.length === 0 && (
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-amber-900">No items to refund</p>
            ...
            {!isLoading && refundableItems.length > 0 && (
              <div className="space-y-2">
                ...
                {refundableItems.map((item) => {
                  ...
                  const refunded = item.refundedAmount ?? 0;
                  const isCounterDisabled = maxQty === 1;
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Row 2 — Qty stepper (always visible) */}
                      <div
                        className={`mt-3 ml-7 flex items-center justify-between transition-opacity duration-150 ${
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={(e) => { e.stopPropagation(); handleDecrement(item); }}
                            disabled={!isSelected || isCounterDisabled || qty <= 1}
                            className="h-8 w-8 rounded-lg bg-white border border-gray-300 flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-100 active:scale-95 transition-all"
                          >
                            <Minus className="h-3.5 w-3.5 text-gray-700" />
                          </button>

                          <span className="min-w-[32px] text-center text-sm font-semibold text-gray-900 tabular-nums select-none">
                            {qty}
                          </span>
                          ...
                          <button
                            ...
                            disabled={!isSelected || isCounterDisabled || qty >= maxQty}
                            className="h-8 w-8 rounded-lg bg-white border border-gray-300 flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-100 active:scale-95 transition-all"
                          >
                            <Plus className="h-3.5 w-3.5 text-gray-700" />
                          </button>

                          <span className="text-xs text-gray-400 tabular-nums">/ {maxQty}</span>
                        </div>
                      </div>
                    </div>
                  );
        ...
        {/* ── Footer ──────────────────────────────────────────────────────── */}
        <div className="bg-white border-t border-gray-200 px-6 py-4 shrink-0 space-y-3">
          <div
          ...
          <Button
            ...
            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Confirm Selection
          </Button>
        </div>
